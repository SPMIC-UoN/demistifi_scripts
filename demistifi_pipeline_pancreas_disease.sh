...
SUBJGROUP=Pancreas_disease

echo "Processing from group $SUBJGROUP"
python3 -u demistifi_pipeline.py --input "${DATA_DIR}" --output "${OUTDIR}/${SUBJGROUP}" --subjids $GROUPSDIR/$SUBJGROUP --subjid-idx=${SLURM_ARRAY_TASK_ID}  --skip-rcoh --skip-renal-preproc
